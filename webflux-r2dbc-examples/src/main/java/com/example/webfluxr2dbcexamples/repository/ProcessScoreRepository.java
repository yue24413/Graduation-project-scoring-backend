...
public interface ProcessScoreRepository extends ReactiveCrudRepository<ProcessScore, String> {
    ...
    @Query("""
      ...
      where ps.student_id=u.id and ps.process_id=:pid 
    ...
    @Modifying
    @Query("""
      update process_score ps set ps.detail=:detail
        where ps.id=:psid;
      """)
    Mono<Integer> updateDetail(String detail,String psid);
...
where ps.student_id=u.id and u.group_number=:groupNumber;
    ...
    @Query("""
        select * from process_score ps ,process p 
          where ps.process_id=p.id and ps.department_id=:depId;
